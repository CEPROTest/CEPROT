...
import logging
from model.codet5_edit_focal import Code2Code
from model.codet5_edit_focal import Config
...
from model.codet5_edit_focal import GenDataset, read_examples
import torch
...
if cofig.do_test:
    ...
    for idx,file in enumerate(files):   
        ...
        ppp = []
        idx = 0
        with open(os.path.join(cofig.output_dir,"fpc_test_epoch{0}.output".format(i)),'w') as f, open(os.path.join(cofig.output_dir,"fpc_test_epoch{0}.gold".format(i)),'w') as f1:
            for ref,gold in zip(p,test_examples):
                f.write(ref+'\n')
                f1.write(gold.test_src+'\n')     
                accs.append(ref==gold.test_src)
                if ref == gold.test_tgt:
                    ppp.append(idx)
                idx += 1
        dev_bleu=round(_bleu(os.path.join(cofig.output_dir, "test_epoch{0}.gold".format(i)), os.path.join(cofig.output_dir, "test_epoch{0}.output".format(i))),2)
        xmatch=round(np.mean(accs)*100,4)
        logger.info("  %s = %s "%("bleu-4",str(dev_bleu)))
        logger.info("  %s = %s "%("xMatch",str(round(np.mean(accs)*100,4))))
        logger.info("  "+"*"*20)    
