...
class NLGMetrics(BaseMetric):
    ...
    def eval(self, hypos: Iterable[List[List[str]]], references: Iterable[List[str]],
        ...
        # distinct
        with open('./re_fpc__result_hypo.txt','w') as f:
            for h in first_hypos:
                f.write(repr(h)[1:-1]+'\n')
        f.close()

        with open('./re_fpc_result_ref.txt','w') as f:
            for h in references_lists:
                for j in h:
                  f.write(repr(j)[1:-1]+'\n')     
        f.close()

        for h in range(len(references_lists)):
            for j in range(len(references_lists[h])):
                references_lists[h][j] = repr(references_lists[h][j])[1:-1]
        for h in range(len(first_hypos)):
            first_hypos[h] = repr(first_hypos[h])[1:-1]
...
class Evaluator(BaseEvaluator):
    ...
    def load_hypos_and_refs(self) -> Tuple[List[List[List[str]]], List[List[str]], List[List[str]]]:
        ...
        references = list(test_set.get_ground_truth())  #get_tgt
