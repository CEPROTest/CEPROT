import jsonlines
from tqdm import tqdm
import json


def writejson(data,filename):
    d = []
    with open(filename,'w') as f:
        for item in tqdm(data):
            d.append(item)
        json.dump(data,f)
    f.close()


def readjson_all(filename):
    d = []
    with open(filename, mode='r', encoding='utf-8') as f:
        dicts = json.load(f)
    for i in dicts:
        d.append(i)
    return d


def readtxt(filename):
    t = []
    with open(filename) as f:
        data = f.readlines()
    for i in data:
        t.append(i.strip())
    return t

def readjs(filename):
    l=[]
    with open(filename, "r+", encoding="utf-8") as f:
        for item in jsonlines.Reader(f):
            l.append(item)
        return l

def writejs(data,filenamewrite): 
    print(str(len(data)) + ' pieces of data will be written in ',filenamewrite)
    with jsonlines.open(filenamewrite, mode='w') as writer:
        for i in tqdm(range(len(data))):
            writer.write(data[i])
    writer.close()
    print('Write Jsonl File Done! Path is '+filenamewrite)

def infer_TPC():
    tpc_hypo_data = []
    tpc_ref_data = []
    TPC_id = [] 
    test_data_c = readjson_all('./class/test.json')
    test_data_g = readjson_all('./gen/test.json')
    hypo_data = readtxt('./CoAttnBPBAUpdater/eval/result_hypo.txt')
    TPC_data = []
    TPC_cnt = 0
    cnt = 0
    c = 0
    for i in tqdm(TPC_id):
        TPC_data.append(test_data_c[i])
    for i in range(len(TPC_data)):
        flag = 1
        tmp = TPC_data[i]['edit_seq']
        for j in range(len(test_data_g)):
            if tmp == test_data_g[j]['edit_seq']:
                if test_data_g[j]['test_tgt'].strip() == test_data_g[j]['test_src'].strip():
                    c+=1
                tpc_hypo_data.append(hypo_data[j])
                tpc_ref_data.append({'tgt':test_data_g[j]['test_tgt'],'src':test_data_g[j]['test_src']})
                tpc_hypo = hypo_data[j]
                if tpc_hypo.strip() == test_data_g[j]['test_tgt'].strip():
                    TPC_cnt += 1
                flag = 0
                break
        if flag == 0:
            cnt += 1
    print("TPC: ",TPC_cnt)
    print("TP: ",len(TPC_id))
if __name__ == '__main__':
    infer_TPC()
    fpc_hypo_data = []
    fpc_ref_data = []
    FPC_id = []    
    test_data_c = readjson_all('./class/test.json')
    infer_data = []
    FPC = 0
    for i in FPC_id:
        infer_data.append(test_data_c[i])
    
    hypo_data = readtxt('./re_fpc__result_hypo.txt')
    assert len(hypo_data) == len(infer_data)
    for i in range(len(infer_data)):
        fpc_hypo_data.append( hypo_data[i])
        fpc_ref_data.append( {'tgt':infer_data[i]['test_src'],'src':infer_data[i]['test_src']})
        if infer_data[i]['test_src'].replace(" ", "") == hypo_data[i].replace(" ", ""):
            FPC += 1
    print(FPC)


    


