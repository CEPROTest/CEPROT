...
class CloneModel(nn.Module):
    ...
    def get_t5_vec(self, source_ids):
        ...
        if len(torch.unique(eos_mask.sum(1))) > 1:
            eos_mask_np = eos_mask.cpu().numpy()
            for i in range(len(eos_mask_np)):
                cnt = 0 
                for j in range(len(eos_mask_np[i])):
                    if eos_mask_np[i][j] == True and cnt != 2:
                        cnt += 1
                    elif eos_mask_np[i][j] == True and cnt == 2:
                        eos_mask_np[i][j] = False
            eos_mask = torch.from_numpy(eos_mask_np).to(device)
