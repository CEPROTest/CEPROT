...
def convert_clone_examples_to_features(item):
    ...
    source_ids =  code_edit + code3
    return CloneInputFeatures(example_index, source_ids, example.label)
...
def read_clone_examples1(filename, data_num):
    import jsonlines
    data = []
    idx = 0
    with open(filename, "r+", encoding="utf-8") as f:
        for item in jsonlines.Reader(f):
            ...
            data.append(CloneExample(item['edit_seq'],item['focal_src'], item['focal_tgt'], item['test_src'], item['label']))
            idx += 1
            if idx == data_num:
                break
    return data
...
def read_clone_examples(filename, data_num):
    ...
    with open(filename, mode='r', encoding='utf-8') as f:
        dicts = json.load(f)
    for item in dicts:
        d.append(CloneExample(item['edit_seq'],repr(item['focal_src'])[1:-1], repr(item['focal_tgt'])[1:-1], repr(item['test_src'])[1:-1], item['label']))
        idx += 1
        if idx == data_num:
            break
    return d
