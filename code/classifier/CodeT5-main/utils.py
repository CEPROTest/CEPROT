...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    examples = read_examples(filename, args.data_num, args.task)
    if is_sample:
        examples = random.sample(examples, int(len(examples) * 0.2))
...
def read_examples(filename, data_num, task):
    ...
    # task = 'clone' -->read_clone_examples(filename, data_num)
...
def calc_stats(examples, tokenizer=None, is_tokenize=False):
    ...
    for ex in examples:
        # print(ex)
        if is_tokenize:
            avg_src_len.append(len(ex.focal_src.split()))
            avg_trg_len.append(len(str(ex.test_src).split()))
            avg_src_len_tokenize.append(len(tokenizer.tokenize(ex.focal_src)))
            avg_trg_len_tokenize.append(len(tokenizer.tokenize(str(ex.test_src))))
        else:
            avg_src_len.append(len(ex.focal_src.split()))
            avg_trg_len.append(len(str(ex.test_src).split()))
    if is_tokenize:
...
def flat_scores(preds, labels, scores):
    ...
    R = float(TP/(TP+FN))
    if scores == 'precision':
        if np.isnan(P) or (TP == 0 and FP == 0):
            return 0
        else:
            return P
    elif scores == 'recall':
        if np.isnan(R) or (TP == 0 and FN == 0):
            return 0
        else:
            return R
    elif scores == 'f1':
